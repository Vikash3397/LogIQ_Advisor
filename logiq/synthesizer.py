...
def persist_resolution(
    ...
    A human-readable ``resolution.txt`` report is written alongside the JSON in
    ...
    written = io_utils.write_json(target, payload)
    report.write_report(run_id, doc=payload)
    return written
